...
class BME280 extends I2C {
  ...
  setSensorSettings(
    ...
    const config =
      (dictionary.standbyTime << 5) | (dictionary.filter << 2) | I2C_INTF;
    const ctrlMeas =
      (dictionary.osrTemperature << 5) |
  ...
  getCalibrationData() {
    ...
    bytes = this.getRegister(
      TEMP_PRESS_CALIB_DATA_ADDR,
  ...
  parseHumidityCalibrationData(regData) {
    const getInt12 = (value) =>
      value & 0x800 ? -((value - 1) ^ 0xfff) : value;
    const regView = new DataView(regData.buffer);
  ...
  compensateHumidity(uncompData) {
    ...
    var2 =
      this.calibData.dig_H4 * 64.0 + (this.calibData.dig_H5 / 16384.0) * var1;
